...
When the `adb logstream` process exits unexpectedly (e.g. device unplugged), the server catches the `close`/`error` event, broadcasts a status message to all clients (`{ "type": "status", "message": "Device disconnected. Retrying..." }`), and automatically restarts `adb logstream` after a 3-second delay.
