...
Provide real-time case-insensitive search across Logcat Entry tags and messages, with visual highlighting of matches.

## Requirements

### Requirement: Search input field

The Client SHALL provide a text input field for entering search queries.
...
The Client SHALL filter Logcat Entries in real time as the user types in the search input.

#### Scenario: User types in search field

- **WHEN** the user types a query into the search field
- **THEN** the log list SHALL update immediately to show only entries whose tag or message contains the query
