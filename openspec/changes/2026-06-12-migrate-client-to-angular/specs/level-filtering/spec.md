...
Allow the user to filter Logcat Entries by Level (V, D, I, W, E, F) using toggle controls in the header, driven by Angular signals.

## Requirements

### Requirement: Level filter controls

The Client SHALL provide six toggle controls — one for each Level: V, D, I, W, E, F, rendered by `LevelTogglesComponent`.
...
### Requirement: Signal-driven filter application

The Client SHALL instantly hide or show all Logcat Entries of a Level when its toggle is switched, via signal reactivity.

#### Scenario: Toggle Verbose off

- **WHEN** the user toggles Verbose off
- **THEN** `LogStateService.levelFilters` signal SHALL update
- **AND** `LogStateService.filteredEntries` computed signal SHALL recompute
- **AND** all Logcat Entries with Level `V` SHALL be hidden from the log list
...
- **THEN** `LogStateService.levelFilters` signal SHALL update
- **AND** `LogStateService.filteredEntries` computed signal SHALL recompute
- **AND** all Logcat Entries with Level `V` SHALL be shown in the log list
...
- **AND** those entries SHALL remain in the in-memory signal
...
- **THEN** the Client SHALL save the state to `LocalStorageService`
- **AND** when the page is reloaded, the Client SHALL restore those states from `LocalStorageService`
