...
Define how Logcat Entries are parsed from WebSocket messages, rendered as rows in the log list via Angular components, and managed with auto-scroll and entry capping.
...
### Requirement: Logcat entry rendering via Angular components

The Client SHALL render each Logcat Entry as a `LogRowComponent` inside a `LogListComponent`.

#### Scenario: Entry rendered

- **WHEN** a Logcat Entry is received
- **THEN** the `LogListComponent` SHALL render a `LogRowComponent` for the entry
