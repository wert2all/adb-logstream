...
Provide users with a toggleable auto-scroll control for the log stream viewer, with state persistence across page reloads, implemented as an Angular component with signals.

## Requirements

### Requirement: Auto-scroll toggle control

The system SHALL provide a checkbox in the footer (`FooterComponent`) that allows the user to toggle auto-scroll on or off.

#### Scenario: Toggle auto-scroll off

- **WHEN** the user clicks the auto-scroll checkbox to uncheck it
- **THEN** `LogStateService.autoScrollEnabled` signal SHALL be set to `false`
...
- **THEN** `LogStateService.autoScrollEnabled` signal SHALL be set to `true`
...
The system SHALL persist the auto-scroll toggle state across page reloads using `LocalStorageService`.
