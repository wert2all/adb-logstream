...
The project needs a server that bridges `adb logstream` output to browser-based clients in real time. Currently the repo has only skeleton code — an empty HTTP server with no adb integration, log parsing, or WebSocket broadcasting. Browser-based debugging of Android devices requires a live stream of structured log data, and this change delivers the server-side foundation to make that possible.

## What Changes

- **adb child process management** — spawn `adb logstream -v long` on startup, monitor for exits, auto-reconnect on device disconnect
- **Logstream entry parser** — parse the `-v long` header+body format into structured JSON (`timestamp`, `pid`, `tid`, `level`, `tag`, `message`)
...
- **Server code**: `/home/wert2all/work/adb-logstream/server/src/index.ts` will be substantially rewritten from placeholder to full implementation (~200-300 lines)
- **Client code**: `/home/wert2all/work/adb-logstream/client/` — the client files are served but their content is out of scope for this change (client-side rendering is a separate concern)
