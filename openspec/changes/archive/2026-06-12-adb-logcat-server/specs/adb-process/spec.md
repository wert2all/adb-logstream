...
### Requirement: Spawn adb logcat on startup
The system SHALL spawn `adb logcat -v long` as a child process on server startup.
...
- **THEN** it SHALL spawn `adb logcat -v long` as a child process
...
- **THEN** `adb logcat -v long` SHALL be spawned again

#### Scenario: Reconnect loop continues indefinitely
- **WHEN** `adb logcat` repeatedly exits after restart
